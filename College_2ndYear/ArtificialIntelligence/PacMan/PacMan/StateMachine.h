...
{
public:
    explicit StateMachine(ATT* initAttribute);
    explicit StateMachine(ATT* initAttribute, std::unique_ptr<State<ATT>>&& entryState);
    ...
    const Transition<ATT>* AddTransition(std::unique_ptr<Transition<ATT>>&& transition, const State<ATT>* from, const State<ATT>* to);
...
template <typename ATT>
StateMachine<ATT>::StateMachine(ATT* initAttribute)
    : mAttribute(initAttribute)
{
}

template<typename ATT>
StateMachine<ATT>::StateMachine(ATT* initAttribute, std::unique_ptr<State<ATT>>&& entryState)
    : mAttribute(initAttribute)
{
...
template<typename ATT>
const Transition<ATT>* StateMachine<ATT>::AddTransition(std::unique_ptr<Transition<ATT>>&& transition, const State<ATT>* from, const State<ATT>* to)
{
    assert(from != nullptr);
    assert(to != nullptr);
    ...
    mSrcTransitionMap[from].emplace_back(transitionPtr);
    mDestTransitionMap[transitionPtr] = const_cast<State<ATT>*>(to);
