...
private:
	...
	void UpdateFlickCB(const GameTimer& gt);
...
void LitWavesApp::Draw(const GameTimer& gt)
{
	...
	auto* flickCB = mCurrFrameResource->FlickCB->Resource();
	mCommandList->SetGraphicsRootConstantBufferView(3, flickCB->GetGPUVirtualAddress());
...
void LitWavesApp::UpdateFlickCB(const GameTimer& gt)
{
	static float PrevTotalTime = 0.0f;
	if (gt.TotalTime() > PrevTotalTime + 1.0f)
	{
		PrevTotalTime = gt.TotalTime();

		if (mFlickCB.GlobalLightIntensity == 0.0f)
		{
			mFlickCB.GlobalLightIntensity = 1.0f;
		}
		else
		{
			mFlickCB.GlobalLightIntensity = 0.0f;
		}
	}

	// Fix global intensity
	//mFlickCB.GlobalLightIntensity = 1.0f;
	
	auto* currentFlickCB = mCurrFrameResource->FlickCB.get();
	currentFlickCB->CopyData(0, mFlickCB);
}
...
void LitWavesApp::BuildFrameResources()
{
    for(int i = 0; i < gNumFrameResources; ++i)
    {
        mFrameResources.push_back(std::make_unique<FrameResource>(md3dDevice.Get(),
            1, (UINT)mAllRitems.size(), (UINT)mMaterials.size(), 1, mWaves->VertexCount()));
    }
}
