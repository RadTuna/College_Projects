...
void BlendApp::UpdateMainPassCB(const GameTimer& gt)
{
	...
	// Fog
	const float fogSineFactor = 300.0f;
	float fogRange = 50.0f;
	fogRange += (std::sinf(gt.TotalTime()) + 1.0f) * fogSineFactor;
	
	mMainPassCB.FogColor = { 0.7f, 0.7f, 0.7f, 1.0f };
	mMainPassCB.gFogStart = 10.0f; // default 5.0f
	mMainPassCB.gFogRange = fogRange; // default 150.0f
...
void BlendApp::BuildRenderItems()
{
	...
	// transparent box
	auto transparentBoxRitem = std::make_unique<RenderItem>();
	XMStoreFloat4x4(&transparentBoxRitem->World, XMMatrixTranslation(13.0f, 2.0f, -19.0f));
	transparentBoxRitem->ObjCBIndex = 3;
	transparentBoxRitem->Mat = mMaterials["ice"].get();
	transparentBoxRitem->Geo = mGeometries["boxGeo"].get();
	transparentBoxRitem->PrimitiveType = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	transparentBoxRitem->IndexCount = transparentBoxRitem->Geo->DrawArgs["box"].IndexCount;
	transparentBoxRitem->StartIndexLocation = transparentBoxRitem->Geo->DrawArgs["box"].StartIndexLocation;
	transparentBoxRitem->BaseVertexLocation = transparentBoxRitem->Geo->DrawArgs["box"].BaseVertexLocation;

	mRitemLayer[(int)RenderLayer::Transparent].push_back(transparentBoxRitem.get());

	// opaque box
	auto opaqueBoxRitem = std::make_unique<RenderItem>();
	XMStoreFloat4x4(&opaqueBoxRitem->World, XMMatrixTranslation(-7.0f, 2.0f, 1.0f));
	opaqueBoxRitem->ObjCBIndex = 4;
	opaqueBoxRitem->Mat = mMaterials["tile"].get();
	opaqueBoxRitem->Geo = mGeometries["boxGeo"].get();
	opaqueBoxRitem->PrimitiveType = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	opaqueBoxRitem->IndexCount = opaqueBoxRitem->Geo->DrawArgs["box"].IndexCount;
	opaqueBoxRitem->StartIndexLocation = opaqueBoxRitem->Geo->DrawArgs["box"].StartIndexLocation;
	opaqueBoxRitem->BaseVertexLocation = opaqueBoxRitem->Geo->DrawArgs["box"].BaseVertexLocation;

	mRitemLayer[(int)RenderLayer::Opaque].push_back(opaqueBoxRitem.get());

    mAllRitems.push_back(std::move(wavesRitem));
    mAllRitems.push_back(std::move(gridRitem));
	mAllRitems.push_back(std::move(boxRitem));
	mAllRitems.push_back(std::move(transparentBoxRitem));
	mAllRitems.push_back(std::move(opaqueBoxRitem));
}
